      ...
      next: (items) => {
        const cartItems = (items || []).map((it: any): CartItem => {
          if (it.type === 'subscription') {
            const discountAmount = it.discountAmount || this.getSubscriptionDiscountAmount({ id: it.subscriptionId, name: it.name, price: it.price, discount: it.discount || 0, discount_type: it.discount_type || 'percentage', duration: it.duration } as Subscription);
            const purchasePrice = it.purchasePrice || this.getSubscriptionDiscountedPrice({ id: it.subscriptionId, name: it.name, price: it.price, discount: it.discount || 0, discount_type: it.discount_type || 'percentage', duration: it.duration } as Subscription);
            return {
